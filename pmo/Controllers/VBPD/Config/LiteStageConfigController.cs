...
namespace pmo.Controllers {
    [Route("vbpd-admin/lite-stage-config")]
    public class LiteStageConfigController : BaseController {
        private readonly string path = "~/Views/VBPD/Config/LiteStageConfig";

        public LiteStageConfigController(EfContext context, IMapper mapper, IHttpContextAccessor httpContextAccessor) : base(context, mapper, httpContextAccessor) {

        }

        public IActionResult Index() {
            var vm = _context.LiteStageConfigs.Include(s=>s.RequiredSchedules).ToList();
            return View($"{path}/Index.cshtml", vm);
        }

        [Route("create")]
        public IActionResult Create() {
            int currentStage = _context.LiteStageConfigs.Count() + 1;
            var stageconfigViewModel = new LiteStageConfigForm() {
        ...
        [Route("create")]
        public IActionResult Create(LiteStageConfigForm model) {
            model.isCreate = true;
            int currentStage = _context.LiteStageConfigs.AsNoTracking().Count() + 1;
            ...
            var domainModel = _mapper.Map<LiteStageConfig>(model);
            ...
            _context.LiteStageConfigs.Add(domainModel);
        ...
        public IActionResult Edit(int id) {
            var config = _context.LiteStageConfigs
                .Include(i => i.GateKeeperConfigs).First(f => f.Id == id);

            if (config == null)
                return NotFound();

            var vm = _mapper.Map<LiteStageConfigForm>(config);
            vm.isCreate = false;
        ...
        [AutoValidateAntiforgeryToken]
        [Route("{id}")]
        public IActionResult Edit(LiteStageConfigForm model) {
            // populate stage number as is from DB
            model.StageNumber = _context.LiteStageConfigs
                .AsNoTracking()
                .First(
            ...
            var passedSchedules = model.ScheduleIds
                .Select(s => new LiteRequiredSchedule {
                    StageConfigId = model.Id,
                    RequiredScheduleTagId = s,
                }).OrderBy(o => o.RequiredScheduleTagId)
            .ToList();

            var existing = _context.LiteRequiredSchedules.AsNoTracking().Where(r => r.StageConfigId == model.Id);
            ...
            if (!Enumerable.SequenceEqual(ids1, ids2)) {
                _context.LiteRequiredSchedules.RemoveRange(existing);
                _context.LiteRequiredSchedules.AddRange(passedSchedules);
                _context.SaveChanges();
            }
        ...
        public IActionResult NewGateKeeper(string keeper, int gateNumber, int stageId) {
            _context.LiteGateKeeperConfigs.Add(new LiteGateKeeperConfig {
                Keeper = keeper,
                StageConfigId = _context.LiteStageConfigs
        ...
        private void SetTagList(LiteStageConfigForm model) {
            var selected = _context.LiteRequiredSchedules
